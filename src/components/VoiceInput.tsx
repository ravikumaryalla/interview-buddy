...
export const VoiceInput: React.FC = () => {
  const { selectedModel, currentSolution, currentProblem, setCredits } =
    useAppStore();
  ...
  }, [liveMsgs, streamingAI, streamingInterviewer]);
  ...
  const connectLive = useCallback(async () => {
    ...
    try {
      ...
      pc.ontrack = () => {
        /* intentionally ignore audio track */
      };
        ...
        try {
          ...
          switch (ev.type) {
            ...
            case 'conversation.item.input_audio_transcription.delta':
              if (ev.delta) setStreamingInterviewer((p) => p + ev.delta);
              break;
            case 'conversation.item.input_audio_transcription.completed':
              ...
              if (ev.transcript?.trim())
                setLiveMsgs((p) => [
                  ...p,
                  { id: uid(), role: 'user', text: ev.transcript.trim() },
                ]);
              break;
      ...
      pc.addEventListener('connectionstatechange', () => {
        if (
          ...
          setLiveStatus('error');
          setLiveError('Connection lost. Try reconnecting.');
        }
      });
              ...
              <div className='flex flex-col items-center justify-center h-full gap-4 select-none'>
                ...
                <div className='text-center'>
                  ...
                  <p className='text-xs text-foreground/30 mt-0.5'>
                    Type, record via mic, or capture speaker output
                  </p>
                </div>
                <div className='flex flex-wrap justify-center gap-1.5'>
                  {QUICK.map((q) => (
                    <button
                      ...
                      onClick={() => sendChat(q)}
                      disabled={isTyping}
                      className='px-2.5 py-1 bg-panel border border-border rounded-full text-[11px] text-foreground/60 hover:text-accent hover:border-accent/40 disabled:opacity-40 transition-colors'
                    >
                      {q}
                    </button>
                  ))}
                            ...
                            components={{
                              code({ className, children }) {
                                ...
                                return match ? (
                                  ...
                                  <code className='bg-black/25 px-1 py-0.5 rounded text-[0.8em] font-mono'>
                                    {children}
                                  </code>
                                );
                              },
            ...
            <div className='flex flex-col items-center justify-center flex-1 gap-5 p-6 select-none'>
              ...
              >
                {liveStatus === 'connecting' ? (
                  ...
                  <AudioWaveform
                    size={28}
                    className={
                      liveStatus === 'error' ? 'text-red-400' : 'text-accent'
                    }
                  />
                )}
              </div>
              ...
              {liveStatus !== 'connecting' && (
                <div className='flex bg-background border border-border rounded-lg p-0.5 w-full max-w-[240px]'>
                  ...
                  <button
                    onClick={() => setLiveAudioSrc('system')}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-all
                ...
                !streamingInterviewer ? (
                  <div className='flex flex-col items-center justify-center h-full gap-3 text-foreground/25 select-none'>
                    ...
                    <p className='text-xs text-center'>
                      {liveAudioSrc === 'mic'
                        ? 'Start speaking — AI is listening'
                        : 'AI is listening to speaker output…'}
                    </p>
                  </div>
                ) : (
                  <>
                    {liveMsgs.map((m) => (
                      ...
                      >
                        ...
                        >
                          {m.role === 'user' && (
                            <span className='text-[10px] text-blue-400/60 block mb-0.5'>
                              {liveAudioSrc === 'system'
                                ? 'Interviewer'
                                : 'You'}
                            </span>
                          ...
                          {m.role === 'ai' && (
                            <span className='text-[10px] text-accent/60 block mb-0.5 flex items-center gap-1'>
                              <Sparkles size={9} /> AI
                            </span>
                          )}
                          {m.role === 'ai' ? (
                      ...
                      <div className='flex justify-end'>
                        <div className='max-w-[85%] px-3 py-2 rounded-2xl rounded-br-sm text-sm bg-blue-500/10 border border-blue-500/20 text-blue-300 leading-relaxed'>
                          <span className='text-[10px] text-blue-400/60 block mb-0.5 flex items-center gap-1'>
                            ...
                            {liveAudioSrc === 'system' ? 'Interviewer' : 'You'}
                          </span>
                          {streamingInterviewer}
                          <span className='inline-block w-1 h-3.5 bg-blue-400/60 ml-0.5 animate-pulse rounded-sm' />
                        </div>
                      </div>
                    )}
