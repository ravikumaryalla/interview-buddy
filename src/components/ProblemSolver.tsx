...
export const ProblemSolver: React.FC = () => {
  const {
    ...
    selectedModel, reasoningEffort, isSolving, setIsSolving,
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3 p-8 select-none">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-foreground/40">Ready to solve</p>
          <p className="text-xs text-foreground/25 mt-1">Capture a screen area to get started</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col overflow-y-auto no-scrollbar">
      <div className="p-3 space-y-3">
        ...
        {/* Extracted text */}
        {currentProblem && (
          <div className="anim-slide-up border border-border rounded-xl overflow-hidden" style={{ boxShadow: 'var(--shadow-sm)' }}>
            <button
        ...
        {promptMode === 'coding' && currentSolution && !isSolving && (
          <div className="anim-fade-in space-y-3">
            ...
            {/* Summary */}
            <div className="bg-panel border border-border rounded-xl p-3.5" style={{ boxShadow: 'var(--shadow-sm)' }}>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground/35 mb-1.5">Summary</p>
              <p className="text-sm text-foreground/80 leading-relaxed">{currentSolution.summary}</p>
            </div>

            {/* Complexity */}
            <div className="grid grid-cols-2 gap-2">
              <div className="flex items-center gap-2.5 bg-panel border border-border rounded-xl p-3 card-hover" style={{ boxShadow: 'var(--shadow-sm)' }}>
                <div className="w-7 h-7 rounded-lg bg-blue-500/10 flex items-center justify-center shrink-0">
                  <Clock size={13} className="text-blue-400" />
                </div>
                <div>
                  <p className="text-[10px] text-foreground/35 font-medium uppercase tracking-wide">Time</p>
                  <p className="text-sm font-mono font-semibold">{currentSolution.complexity.time}</p>
                </div>
              </div>
              <div className="flex items-center gap-2.5 bg-panel border border-border rounded-xl p-3 card-hover" style={{ boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div>
                  <p className="text-[10px] text-foreground/35 font-medium uppercase tracking-wide">Space</p>
                  <p className="text-sm font-mono font-semibold">{currentSolution.complexity.space}</p>
                </div>
              </div>
            </div>
            ...
            {/* Approach */}
            <div className="bg-panel border border-border rounded-xl p-3.5" style={{ boxShadow: 'var(--shadow-sm)' }}>
              <p className="text-[10px] font-semibold uppercase tracking-wider text-foreground/35 mb-1.5">Approach</p>
            ...
            <div className="border border-border rounded-xl overflow-hidden" style={{ boxShadow: 'var(--shadow-sm)' }}>
              <div className="bg-panel border-b border-border px-2 pt-2">
                <Tabs value={selectedLanguage} onValueChange={(v) => setSelectedLanguage(v as 'javascript' | 'python' | 'java')}>
                  <TabsList className="h-7">
                    <TabsTrigger value="javascript" className="h-6 px-3 text-[11px]">JS</TabsTrigger>
                    <TabsTrigger value="python"     className="h-6 px-3 text-[11px]">Python</TabsTrigger>
                    <TabsTrigger value="java"       className="h-6 px-3 text-[11px]">Java</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
              {(currentSolution.code as any)[selectedLanguage] ? (
                ...
                <p className="p-4 text-center text-xs text-foreground/35 italic">No code for {selectedLanguage}</p>
              )}
            </div>
          </div>
        )}
